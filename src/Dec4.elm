...
one : String -> Int
one =
    prepareInput >> List.length


two : String -> Int
two =
    prepareInput >> List.count isValid


prepareInput : String -> List Passport
prepareInput =
    let
        toPair strings =
            Maybe.map2 Tuple.pair (List.head strings) (Maybe.andThen List.head (List.tail strings))
    in
    String.split "\n\n"
        >> List.map (String.split "\n")
        >> List.map (List.concatMap (String.split " "))
        >> List.filterMap (List.map (String.split ":") >> List.filterMap toPair >> Dict.fromList >> toPassport)


toPassport : Dict String String -> Maybe Passport
toPassport dict =
    Just Passport
        |> Maybe.andMap (Dict.get "byr" dict)
        |> Maybe.andMap (Dict.get "iyr" dict)
        |> Maybe.andMap (Dict.get "eyr" dict)
        |> Maybe.andMap (Dict.get "hgt" dict)
        |> Maybe.andMap (Dict.get "hcl" dict)
        |> Maybe.andMap (Dict.get "ecl" dict)
        |> Maybe.andMap (Dict.get "pid" dict)
        |> Maybe.andMap (Dict.get "cid" dict |> Just)
...
between low high x =
    Basics.clamp low high x == x


validBYR : String -> Bool
validBYR =
    String.toInt >> Maybe.map (between 1920 2002) >> Maybe.withDefault False


validIYR : String -> Bool
validIYR =
    String.toInt >> Maybe.map (between 2010 2020) >> Maybe.withDefault False


validEYR : String -> Bool
validEYR =
    String.toInt >> Maybe.map (between 2020 2030) >> Maybe.withDefault False


validHGT : String -> Bool
validHGT str =
    if String.endsWith "cm" str then
        String.toInt (String.dropRight 2 str) |> Maybe.map (between 150 193) |> Maybe.withDefault False

    else if String.endsWith "in" str then
        String.toInt (String.dropRight 2 str) |> Maybe.map (between 59 76) |> Maybe.withDefault False
...
validHCL str =
    String.startsWith "#" str
        && (Hex.fromString (String.dropLeft 1 str) |> Result.map (always True) |> Result.withDefault False)
...
validPID str =
    String.all Char.isDigit str && String.length str == 9
