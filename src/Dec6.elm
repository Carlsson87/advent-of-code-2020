...
one =
    ...
    in
    prepareInput >> List.map countAnswers >> List.sum
...
two =
    let
        countAnswers =
            List.foldl Set.intersect allQuestions >> Set.size
    in
    prepareInput >> List.map countAnswers >> List.sum


allQuestions : Set Char
allQuestions =
    List.range (Char.toCode 'a') (Char.toCode 'z') |> List.map Char.fromCode |> Set.fromList


prepareInput : String -> List (List (Set Char))
prepareInput =
