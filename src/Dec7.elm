...
type alias Rule =
    { bag : String
    , canContain : List ( Int, String )
    }


one : String -> Int
one str =
    findPlaces (prepareInput str) "shiny gold"
        |> List.length


two : String -> Int
two str =
    countBags (prepareInput str) "shiny gold"


findPlaces : Dict String (List ( Int, String )) -> String -> List String
findPlaces dict bag =
    let
        canFitInBagsDirectly =
            Dict.filter (\_ contains -> List.member bag (List.map Tuple.second contains)) dict
                |> Dict.keys

        canFitInBagsIndirectly =
            List.concatMap (findPlaces dict) canFitInBagsDirectly
    in
    canFitInBagsDirectly
        ++ canFitInBagsIndirectly
        |> List.unique
...
countBags : Dict String (List ( Int, String )) -> String -> Int
countBags dict bag =
    let
        countInner ( n, other ) =
            n + (n * countBags dict other)
    in
    case Dict.get bag dict of
        Just other ->
            List.sum (List.map countInner other)

        Nothing ->
            0


prepareInput : String -> Dict String (List ( Int, String ))
prepareInput str =
    String.lines str
        |> List.map stringToRule
        |> Dict.fromList


stringToRule : String -> ( String, List ( Int, String ) )
stringToRule str =
    let
        toPair str2 =
            case String.toInt (String.left 1 str2) of
                Just n ->
                    Just ( n, String.split " " str2 |> List.drop 1 |> List.take 2 |> String.join " " )

                Nothing ->
                    Nothing
    in
    ( String.split " " str |> List.take 2 |> String.join " "
    , String.split " " str
        |> List.drop 4
        |> String.join " "
        |> String.split ","
        |> List.filterMap (String.trim >> toPair)
    )
